...
local MockLib = {}
function MockLib:MakeWindow(options)
    local title = options.Title or options[".Title"] or "Hub"
    local Window = Fluent:CreateWindow({
        Title = title,
        SubTitle = "Remade by Antigravity",
        TabWidth = 160,
    ...
    function WindowWrapper:MakeTab(options)
        ...
        function TabWrapper:AddSection(options)
             local name = options[1] or "Section"
             Tab:AddSection(name)
             return TabWrapper
        end
        function TabWrapper:AddToggle(options)
             local title = options.Title or options[".Title"] or options.Name or options[".Name"] or "Toggle"
             local desc = options.Jungle or options[".Jungle"] or ""
             local val = options.Value or options[".Value"] or options.Default or options[".Default"] or false
             local cb = options.Callback or options[".Callback"] or function() end
             Tab:AddToggle(title, {Title = title, Default = val, Callback = cb, Description = desc })
        end
        function TabWrapper:AddButton(options)
             local title = options.Title or options[".Title"] or options.Name or options[".Name"] or "Button"
             local desc = options.Jungle or options[".Jungle"] or ""
             local cb = options.Callback or options[".Callback"] or function() end
             Tab:AddButton({Title = title, Description = desc, Callback = cb })
        end
        function TabWrapper:AddParagraph(options)
            local title = options.Title or options[".Title"] or "Info"
            local content = options.Content or options[".Content"] or ""
            Tab:AddParagraph({Title = title, Content = content})
        end
        function TabWrapper:AddDropdown(options)
             local title = options.Title or options[".Title"] or options.Name or options[".Name"] or "Dropdown"
             local list = options.Options or options[".Options"] or {}
             local def = options.Default or options[".Default"] or list[1]
             local cb = options.Callback or options[".Callback"] or function() end
             Tab:AddDropdown(title, {Title = title, Values = list, Default = def, Multi = false, Callback = cb})
        end
        function TabWrapper:AddSlider(options)
             local title = options.Title or options[".Title"] or "Slider"
             local min = options.Min or options[".Min"] or 0
             local max = options.Max or options[".Max"] or 100
             local def = options.Default or options[".Default"] or min
             local cb = options.Callback or options[".Callback"] or function() end
             Tab:AddSlider(title, {Title = title, Min = min, Max = max, Default = def, Callback = cb})
        end
        return TabWrapper
    end
    function WindowWrapper:Minimize(val)
        -- Placeholder to prevent error. User should use Keybind.
    end
    return WindowWrapper
end
P[165] = MockLib:MakeWindow({Title="NaNa TV Hub Remade"})P[112]=Instance.new("ScreenGui")P[112].Name="ControlGUI"P[112].Parent=game.CoreGuiP[57]=Instance.new("ImageButton")P[57].Size=UDim2.new(0,50,0,50)P[57].Position=UDim2.new(.15,0,.15,0)P[57].Image="rbxassetid://92120582532478"P[57].BackgroundTransparency=1P[57].Parent=P[112]P[66]=Instance.new("UICorner")P[66].CornerRadius=UDim.new(0.25,0)P[66].Parent=P[57]P[71]=Instance.new("UIStroke")P[71].Thickness=2 P[71].ApplyStrokeMode=Enum.ApplyStrokeMode.BorderP[71].Parent=P[57]P[122]={Color3.fromRGB(255,0,0),Color3.fromRGB(255,127,0);
